...
def dynamic_edge_bps(seconds_left, max_seconds=30):
    # Larger edge early, smaller edge near expiry
    if seconds_left is None:
        return MAX_EDGE_BPS
    ratio = max(0.0, min(1.0, seconds_left / max_seconds))
    return MIN_EDGE_BPS + (MAX_EDGE_BPS - MIN_EDGE_BPS) * ratio
...
def main():
    ...
    while True:
        ...
        for t in tenders:
            ...
            seconds_left = None
            if expires is not None and current_tick is not None:
                seconds_left = max(0, expires - current_tick)

            edge_bps = dynamic_edge_bps(seconds_left)
