"""RITC 2026 liquidity bot hybridized from final_risky + liquidityBot_tenders."""
...
# Strategy params.
# Each setting prefers RIT_LIQ2026_* and falls back to prior final_risky env names.
MIN_EDGE = float(_env("RIT_LIQ2026_MIN_EDGE", "RIT_FINAL_RISKY_MIN_EDGE", "0.08"))
VOL_FACTOR = float(_env("RIT_LIQ2026_VOL_FACTOR", "RIT_FINAL_RISKY_VOL_FACTOR", "0.95"))
MAX_ATTEMPTS = int(_env("RIT_LIQ2026_MAX_ATTEMPTS", "RIT_FINAL_RISKY_MAX_ATTEMPTS", "16"))
EVAL_DELAY = float(_env("RIT_LIQ2026_EVAL_DELAY", "RIT_FINAL_RISKY_EVAL_DELAY", "0.8"))
ORDER_DELAY = float(_env("RIT_LIQ2026_ORDER_DELAY", "RIT_FINAL_RISKY_ORDER_DELAY", "0.10"))
AFTER_ACCEPT_DELAY = float(_env("RIT_LIQ2026_AFTER_ACCEPT_DELAY", "RIT_FINAL_RISKY_AFTER_ACCEPT_DELAY", "0.35"))
MAX_ORDER_SIZE = 10000.0
ENDGAME_TICKS = int(_env("RIT_LIQ2026_ENDGAME_TICKS", "RIT_FINAL_RISKY_ENDGAME_TICKS", "5"))
FIXED_ONLY = _env_bool("RIT_LIQ2026_FIXED_ONLY", "RIT_FINAL_RISKY_FIXED_ONLY", "0")
AGGRESSIVE_MODE = _env_bool("RIT_LIQ2026_AGGRESSIVE", "RIT_FINAL_RISKY_AGGRESSIVE", "1")
EDGE_FLOOR_RATIO = float(_env("RIT_LIQ2026_EDGE_FLOOR_RATIO", "RIT_FINAL_RISKY_EDGE_FLOOR_RATIO", "0.30"))
EDGE_DECAY_PER_ATTEMPT = float(_env("RIT_LIQ2026_EDGE_DECAY_PER_ATTEMPT", "RIT_FINAL_RISKY_EDGE_DECAY_PER_ATTEMPT", "0.015"))
VOL_RELAX_PER_ATTEMPT = float(_env("RIT_LIQ2026_VOL_RELAX_PER_ATTEMPT", "RIT_FINAL_RISKY_VOL_RELAX_PER_ATTEMPT", "0.08"))
HEDGE_RATIO = float(_env("RIT_LIQ2026_HEDGE_RATIO", "RIT_FINAL_RISKY_HEDGE_RATIO", "0.45" if AGGRESSIVE_MODE else "0.80"))
HEDGE_RATIO = max(0.0, min(1.0, HEDGE_RATIO))
LEGACY_THRESHOLD = float(_env("RIT_LIQ2026_LEGACY_THRESHOLD", "RIT_FINAL_MIN_EDGE", "0.15"))
LEGACY_VOL_MULTIPLIER = float(_env("RIT_LIQ2026_LEGACY_VOL_MULTIPLIER", "RIT_FINAL_VOL_FACTOR", "1.2"))
PORTFOLIO_PRINT_INTERVAL = float(_env("RIT_LIQ2026_PORTFOLIO_PRINT_INTERVAL", "RIT_FINAL_RISKY_PORTFOLIO_PRINT_INTERVAL", "5.0"))
TAKE_PROFIT_ENABLED = _env_bool("RIT_LIQ2026_TAKE_PROFIT_ENABLED", "RIT_FINAL_RISKY_TAKE_PROFIT_ENABLED", "1")
TAKE_PROFIT_PER_SHARE = float(_env("RIT_LIQ2026_TAKE_PROFIT_PER_SHARE", "RIT_FINAL_RISKY_TAKE_PROFIT_PER_SHARE", "0.30"))
TAKE_PROFIT_CHUNK_QTY = float(_env("RIT_LIQ2026_TAKE_PROFIT_CHUNK_QTY", "RIT_FINAL_RISKY_TAKE_PROFIT_CHUNK_QTY", "10000"))
TAKE_PROFIT_CHUNK_QTY = max(1.0, TAKE_PROFIT_CHUNK_QTY)
TAKE_PROFIT_COOLDOWN = float(_env("RIT_LIQ2026_TAKE_PROFIT_COOLDOWN", "RIT_FINAL_RISKY_TAKE_PROFIT_COOLDOWN", "2.0"))
SAVE_REPORT_ON_EXIT = _env_bool("RIT_LIQ2026_SAVE_REPORT_ON_EXIT", "RIT_FINAL_RISKY_SAVE_REPORT_ON_EXIT", "1")
REPORT_PREFIX = _env("RIT_LIQ2026_REPORT_PREFIX", "RIT_FINAL_RISKY_REPORT_PREFIX", "liquidity_ritc_2026_hybrid_report")
...
def save_run_report(session, reason, run_error=None):
    ...
    report = {
        ...
        "config": {
            ...
            "LEGACY_VOL_MULTIPLIER": LEGACY_VOL_MULTIPLIER,
...
def _action_edge_ok(action, tender_price, vwap_bid, vwap_ask, bid_volume, ask_volume, attempt_idx):
    """
    Hybrid accept rule:
    1) final_risky dynamic edge/volume rule.
    2) liquidityBot_tenders legacy threshold/volume rule.
    3) Economic fallback for feeds where BUY/SELL semantics are inverted.
    """
    eff_edge = MIN_EDGE
    ...
    if action == "BUY":
        cond_final_risky = (tender_price < (vwap_bid + eff_edge)) and (bid_volume * eff_vol_factor > ask_volume)
        cond_legacy = (tender_price < (vwap_bid + LEGACY_THRESHOLD)) and (bid_volume * LEGACY_VOL_MULTIPLIER > ask_volume)
        cond_fallback = (tender_price - vwap_ask) >= eff_edge
        return cond_final_risky or cond_legacy or cond_fallback
    if action == "SELL":
        cond_final_risky = (tender_price > (vwap_ask - eff_edge)) and (ask_volume * eff_vol_factor > bid_volume)
        cond_legacy = (tender_price > (vwap_ask - LEGACY_THRESHOLD)) and (ask_volume * LEGACY_VOL_MULTIPLIER > bid_volume)
