...
BOOK_LEVEL_LIMIT = int(_env("RIT_FINAL_RISKY_BOOK_LEVEL_LIMIT", "RIT_FINAL_BOOK_LEVEL_LIMIT", "25"))
BOOK_LEVEL_LIMIT = max(1, BOOK_LEVEL_LIMIT)
BOOK_MIN_QTY = float(_env("RIT_FINAL_RISKY_BOOK_MIN_QTY", "RIT_FINAL_BOOK_MIN_QTY", "1000"))
BOOK_MIN_QTY = max(0.0, BOOK_MIN_QTY)
BOOK_MIN_FILL_RATIO = float(_env("RIT_FINAL_RISKY_BOOK_MIN_FILL_RATIO", "RIT_FINAL_BOOK_MIN_FILL_RATIO", "0.85"))
BOOK_MIN_FILL_RATIO = max(0.05, min(1.0, BOOK_MIN_FILL_RATIO))
SAVE_REPORT_ON_EXIT = _env_bool("RIT_FINAL_RISKY_SAVE_REPORT_ON_EXIT", "RIT_FINAL_SAVE_REPORT_ON_EXIT", "1")
REPORT_PREFIX = _env("RIT_FINAL_RISKY_REPORT_PREFIX", "RIT_FINAL_REPORT_PREFIX", "final_risky_depth_qty_report")
...
def get_order_book_agg(session, ticker):
    ...
    for tk in _related_tickers(session, ticker):
        r = session.get(f"{BASE_URL}/securities/book", params={"ticker": tk, "limit": BOOK_LEVEL_LIMIT})
        if r.ok:
            books[tk] = r.json()
...
def _action_edge_ok(action, tender_price, ob, tender_qty, attempt_idx):
    """
    Accept based on executable depth for tender size:
    BUY tender: we need to unwind by selling into bids.
    SELL tender: we need to unwind by buying from asks.
    """
    eff_edge = MIN_EDGE
    if AGGRESSIVE_MODE:
        eff_edge = max(MIN_EDGE * EDGE_FLOOR_RATIO, MIN_EDGE - (EDGE_DECAY_PER_ATTEMPT * attempt_idx))

    target_qty = max(1.0, float(tender_qty))
    if action == "BUY":
        exec_px, filled = _weighted_exec_price(ob["bids"], target_qty)
        fill_ratio = (filled / target_qty) if target_qty > 0 else 0.0
        if exec_px is None or fill_ratio < BOOK_MIN_FILL_RATIO:
            return False, None, fill_ratio, eff_edge
        edge = exec_px - tender_price
        return edge >= eff_edge, edge, fill_ratio, eff_edge
    if action == "SELL":
        exec_px, filled = _weighted_exec_price(ob["asks"], target_qty)
        fill_ratio = (filled / target_qty) if target_qty > 0 else 0.0
        if exec_px is None or fill_ratio < BOOK_MIN_FILL_RATIO:
            return False, None, fill_ratio, eff_edge
        edge = tender_price - exec_px
        return edge >= eff_edge, edge, fill_ratio, eff_edge
    return False, None, 0.0, eff_edge
...
def evaluate_tender(session, tender):
    ...
    if tender_qty <= 0:
        print(f"Tender {tid} missing quantity; declining.")
        decline_tender(session, tender)
        return
    ...
    for i in range(attempts):
        ...
        live_qty = _tender_quantity(live) or tender_qty
        live_action = str(live.get("action", action)).upper()

        ob = get_order_book_agg(session, ticker)
        edge_ok, edge, fill_ratio, eff_edge = _action_edge_ok(
            live_action,
            tender_price,
            ob,
            live_qty,
        ...
        print(
            ...
            f"px={tender_price:.2f} qty={live_qty:.0f} "
            f"edge={edge if edge is not None else 'N/A'} "
            f"fill={fill_ratio:.2f} req_edge={eff_edge:.3f}"
        )
