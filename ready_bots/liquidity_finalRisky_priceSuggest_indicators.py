...
REPORT_PREFIX = _env("RIT_FINAL_RISKY_REPORT_PREFIX", "RIT_FINAL_REPORT_PREFIX", "liquidity_finalRisky_priceSuggest_indicators_report")
INDICATOR_ENABLE = _env_bool("RIT_FINAL_RISKY_INDICATOR_ENABLE", "RIT_FINAL_INDICATOR_ENABLE", "1")
INDICATOR_GATE_STRICT = _env_bool("RIT_FINAL_RISKY_INDICATOR_GATE_STRICT", "RIT_FINAL_INDICATOR_GATE_STRICT", "1")
IND_HISTORY_LIMIT = int(_env("RIT_FINAL_RISKY_IND_HISTORY_LIMIT", "RIT_IND_HISTORY_LIMIT", "120"))
IND_EMA_FAST = int(_env("RIT_FINAL_RISKY_IND_EMA_FAST", "RIT_IND_EMA_FAST", "8"))
IND_EMA_SLOW = int(_env("RIT_FINAL_RISKY_IND_EMA_SLOW", "RIT_IND_EMA_SLOW", "21"))
IND_RSI_PERIOD = int(_env("RIT_FINAL_RISKY_IND_RSI_PERIOD", "RIT_IND_RSI_PERIOD", "14"))
IND_VOL_PERIOD = int(_env("RIT_FINAL_RISKY_IND_VOL_PERIOD", "RIT_IND_VOL_PERIOD", "20"))
IND_RSI_OVERBOUGHT = float(_env("RIT_FINAL_RISKY_IND_RSI_OVERBOUGHT", "RIT_IND_RSI_OVERBOUGHT", "70"))
IND_RSI_OVERSOLD = float(_env("RIT_FINAL_RISKY_IND_RSI_OVERSOLD", "RIT_IND_RSI_OVERSOLD", "30"))
IND_VOL_SUPPORT = float(_env("RIT_FINAL_RISKY_IND_VOL_SUPPORT", "RIT_IND_VOL_SUPPORT", "0.95"))
IND_EDGE_FAVORABLE_MULT = float(_env("RIT_FINAL_RISKY_IND_EDGE_FAVORABLE_MULT", "RIT_IND_EDGE_FAVORABLE_MULT", "0.85"))
IND_EDGE_UNFAVORABLE_MULT = float(_env("RIT_FINAL_RISKY_IND_EDGE_UNFAVORABLE_MULT", "RIT_IND_EDGE_UNFAVORABLE_MULT", "1.20"))
IND_HEDGE_FAVORABLE_DELTA = float(_env("RIT_FINAL_RISKY_IND_HEDGE_FAVORABLE_DELTA", "RIT_IND_HEDGE_FAVORABLE_DELTA", "-0.15"))
IND_HEDGE_UNFAVORABLE_DELTA = float(_env("RIT_FINAL_RISKY_IND_HEDGE_UNFAVORABLE_DELTA", "RIT_IND_HEDGE_UNFAVORABLE_DELTA", "0.20"))
IND_TP_FAVORABLE_MULT = float(_env("RIT_FINAL_RISKY_IND_TP_FAVORABLE_MULT", "RIT_IND_TP_FAVORABLE_MULT", "1.20"))
IND_TP_UNFAVORABLE_MULT = float(_env("RIT_FINAL_RISKY_IND_TP_UNFAVORABLE_MULT", "RIT_IND_TP_UNFAVORABLE_MULT", "0.80"))
IND_CACHE_TTL = float(_env("RIT_FINAL_RISKY_IND_CACHE_TTL", "RIT_IND_CACHE_TTL", "1.0"))
...
def get_indicator_state(session, ticker):
    if not INDICATOR_ENABLE:
        return {"ready": False, "ema_fast": 0.0, "ema_slow": 0.0, "rsi": 50.0, "vol_ratio": 1.0}

    now = time.monotonic()
    cached = IND_CACHE.get(ticker)
    if cached and (now - cached["ts"] <= IND_CACHE_TTL):
        return cached["state"]

    closes = []
    vols = []
    try:
        r = session.get(f"{BASE_URL}/securities/history", params={"ticker": ticker, "limit": IND_HISTORY_LIMIT})
        if r.ok:
            closes, vols = _extract_history_points(r.json())
    except Exception:
        pass

    if len(closes) < max(IND_EMA_SLOW + 3, IND_RSI_PERIOD + 3):
        state = {"ready": False, "ema_fast": 0.0, "ema_slow": 0.0, "rsi": 50.0, "vol_ratio": 1.0}
    else:
        ema_fast = _ema(closes, IND_EMA_FAST)
        ema_slow = _ema(closes, IND_EMA_SLOW)
        rsi_val = _rsi(closes, IND_RSI_PERIOD)
        recent_vol = vols[-1] if vols else 0.0
        vol_window = vols[-IND_VOL_PERIOD:] if len(vols) >= IND_VOL_PERIOD else vols
    ...
    IND_CACHE[ticker] = {"ts": now, "state": state}
    return state


def _direction_from_tender_action(action):
    if action == "SELL":
        return "LONG"
    if action == "BUY":
        return "SHORT"
...
def _direction_from_position(pos):
    if pos > 0:
        return "LONG"
    if pos < 0:
        return "SHORT"
...
def _indicator_regime(ind, direction):
    if not INDICATOR_ENABLE or not ind.get("ready") or direction == "NEUTRAL":
        return "neutral"
    ...
    vol_ok = vol_ratio >= IND_VOL_SUPPORT

    if direction == "LONG":
        if trend_up and rsi_val < IND_RSI_OVERBOUGHT and vol_ok:
            return "favorable"
        if trend_down or rsi_val >= IND_RSI_OVERBOUGHT:
            return "unfavorable"
    elif direction == "SHORT":
        if trend_down and rsi_val > IND_RSI_OVERSOLD and vol_ok:
            return "favorable"
        if trend_up or rsi_val <= IND_RSI_OVERSOLD:
            return "unfavorable"
    return "neutral"


def _indicator_accept_gate(ind, direction):
    regime = _indicator_regime(ind, direction)
    if not INDICATOR_ENABLE or not INDICATOR_GATE_STRICT or direction == "NEUTRAL":
        return True, regime
    return regime == "favorable", regime


def _edge_mult_for_regime(regime):
    if regime == "favorable":
        return IND_EDGE_FAVORABLE_MULT
    if regime == "unfavorable":
        return IND_EDGE_UNFAVORABLE_MULT
    return 1.0


def _dynamic_hedge_ratio(base_ratio, regime):
    out = base_ratio
    if regime == "favorable":
        out += IND_HEDGE_FAVORABLE_DELTA
    elif regime == "unfavorable":
        out += IND_HEDGE_UNFAVORABLE_DELTA
    return max(0.0, min(1.0, out))


def _dynamic_take_profit_threshold(base_threshold, regime):
    if regime == "favorable":
        return base_threshold * IND_TP_FAVORABLE_MULT
    if regime == "unfavorable":
        return base_threshold * IND_TP_UNFAVORABLE_MULT
...
def save_run_report(session, reason, run_error=None):
    ...
    report = {
        ...
        "config": {
            ...
            "INDICATOR_GATE_STRICT": INDICATOR_GATE_STRICT,
            "IND_EMA_FAST": IND_EMA_FAST,
            "IND_EMA_SLOW": IND_EMA_SLOW,
            "IND_RSI_PERIOD": IND_RSI_PERIOD,
            "IND_VOL_PERIOD": IND_VOL_PERIOD,
            "IND_RSI_OVERBOUGHT": IND_RSI_OVERBOUGHT,
            "IND_RSI_OVERSOLD": IND_RSI_OVERSOLD,
            "IND_VOL_SUPPORT": IND_VOL_SUPPORT,
            "IND_EDGE_FAVORABLE_MULT": IND_EDGE_FAVORABLE_MULT,
            "IND_EDGE_UNFAVORABLE_MULT": IND_EDGE_UNFAVORABLE_MULT,
            "IND_HEDGE_FAVORABLE_DELTA": IND_HEDGE_FAVORABLE_DELTA,
            "IND_HEDGE_UNFAVORABLE_DELTA": IND_HEDGE_UNFAVORABLE_DELTA,
            "IND_TP_FAVORABLE_MULT": IND_TP_FAVORABLE_MULT,
            "IND_TP_UNFAVORABLE_MULT": IND_TP_UNFAVORABLE_MULT,
        },
        "case": case_info,
        "trader": trader_info,
        "limits": limits_info,
...
def accept_tender(session, tender, submit_price=None):
    ...
    params = None
    if isinstance(submit_price, (int, float)):
        params = {"price": round(float(submit_price), 2)}
    r = session.post(f"{BASE_URL}/tenders/{tid}", params=params)
    if not r.ok:
        print(f"Accept failed tender {tid}: status={r.status_code}")
        return False
    if isinstance(submit_price, (int, float)):
        print(
            f"Accepted Tender {tid}: {tender.get('ticker')} {tender.get('action')} "
            f"submit_price={float(submit_price):.2f}"
        )
    else:
...
def maybe_take_profit_cover(session, last_tp_by_ticker):
    ...
    for s in get_securities(session):
        ...
        pos_direction = _direction_from_position(pos)
        ind = get_indicator_state(session, ticker)
        regime = _indicator_regime(ind, pos_direction)
        tp_threshold = _dynamic_take_profit_threshold(TAKE_PROFIT_PER_SHARE, regime)
        ...
        print(
            ...
            f"edge={edge:.3f} thr={tp_threshold:.3f} regime={regime} "
            f"cost={cost:.2f} exec={exec_px:.2f}"
        )
...
def _effective_accept_params(attempt_idx, regime="neutral"):
    eff_edge = MIN_EDGE
    ...
    eff_edge = max(0.01, eff_edge * _edge_mult_for_regime(regime))
    return eff_edge, eff_vol_factor


def _depth_vwap(levels, quantity):
    ...
    for lvl in levels:
        px = lvl.get("price")
        q = lvl.get("quantity")
        if not isinstance(px, (int, float)) or not isinstance(q, (int, float)) or q <= 0:
            continue
        take = min(qty_needed - filled, float(q))
        if take <= 0:
            break
        filled += take
        notional += take * float(px)
        if filled >= qty_needed:
            break
    if filled < qty_needed or filled <= 0:
        return None
...
def _auction_volume_ok(action, bid_volume, ask_volume, eff_vol_factor):
    if action == "BUY":
        return bid_volume * eff_vol_factor > ask_volume
    if action == "SELL":
        return ask_volume * eff_vol_factor > bid_volume
    return False


def _suggest_submit_price(action, quantity, ob, edge):
    if action == "BUY":
        hedge_vwap = _depth_vwap(ob.get("bids", []), quantity)
        if hedge_vwap is None:
            return None
        return round(max(0.01, hedge_vwap - edge), 2)
    if action == "SELL":
        hedge_vwap = _depth_vwap(ob.get("asks", []), quantity)
        if hedge_vwap is None:
            return None
        return round(max(0.01, hedge_vwap + edge), 2)
    return None
...
def _action_edge_ok(action, tender_price, vwap_bid, vwap_ask, bid_volume, ask_volume, attempt_idx, regime):
    """
    Hybrid accept rule:
    ...
    """
    eff_edge, eff_vol_factor = _effective_accept_params(attempt_idx, regime=regime)
...
def evaluate_tender(session, tender):
    ...
    if FIXED_ONLY and not is_fixed:
        ...
        return
    if not is_fixed and tender_qty <= 0:
        decline_tender(session, tender)
        return
    if is_fixed and not isinstance(raw_price, (int, float)):
        decline_tender(session, tender)
        return

    tender_price = float(raw_price) if isinstance(raw_price, (int, float)) else None
    ...
    for i in range(attempts):
        ...
        ob = get_order_book_agg(session, ticker)
        tender_direction = _direction_from_tender_action(action)
        ind = get_indicator_state(session, ticker)
        allow_by_indicator, regime = _indicator_accept_gate(ind, tender_direction)
        if not allow_by_indicator:
            print(
                f"Evaluating tender {tid} ({i + 1}/{attempts}) "
                f"blocked_by_indicators dir={tender_direction} "
                f"ema_fast={ind.get('ema_fast', 0.0):.3f} ema_slow={ind.get('ema_slow', 0.0):.3f} "
                f"rsi={ind.get('rsi', 50.0):.1f} volr={ind.get('vol_ratio', 1.0):.2f}"
            )
            continue
        ...
        if is_fixed:
            ...
            eff_edge, eff_vol_factor = _effective_accept_params(i, regime=regime)
            submit_price = _suggest_submit_price(action, tender_qty, ob, eff_edge)
            vol_ok = _auction_volume_ok(action, ob["bid_volume"], ob["ask_volume"], eff_vol_factor)
            accept_now = submit_price is not None and vol_ok

        if accept_now:
            pre = get_inventory_total(session, ticker)
            accepted = accept_tender(session, tender, submit_price=submit_price)
            if accepted:
                ...
                if abs(delta) > 0:
                    hedge_ratio_now = _dynamic_hedge_ratio(HEDGE_RATIO, regime)
                    ...
                    if abs(retained) >= 1:
                        print(
                            ...
                            f"(hedge_ratio={hedge_ratio_now:.2f}, regime={regime})"
                        )
            break
        if is_fixed:
            print(
            ...
            sp = "NA" if submit_price is None else f"{submit_price:.2f}"
            print(
                f"Evaluating tender {tid} ({i + 1}/{attempts}) "
                f"auction_sp={sp} qty={tender_qty:.0f} vb={ob['vwap_bid']:.2f} va={ob['vwap_ask']:.2f}"
            )
